      ...
      <div className="max-w-7xl mx-auto w-full relative z-10">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-20 gap-8">
          <div>
            ...
            >
              System <br /><span className="text-black/20 italic">Design.</span>
            </motion.h2>
          </div>
          <motion.p
            initial={{ opacity: 0 }}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 md:gap-12">
          {EXPERTISE.map((item, index) => (
            <motion.div
              ...
              className="relative aspect-square rounded-full glass bg-white/40 border border-black/[0.04] p-8 flex items-center justify-center text-center group cursor-none interactive"
            >
              ...
              {/* Orbital detail ring */}
              <motion.div 
                className="absolute inset-0 rounded-full border border-blue-500/0 group-hover:border-blue-500/10 scale-100 group-hover:scale-115 transition-all duration-1000 pointer-events-none"
              />
            </motion.div>
          ))}
        </div>
